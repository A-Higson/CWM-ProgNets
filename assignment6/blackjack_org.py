...
def main():
	...
	counter = 0
    while True:
        s = input('Enter 0 to start STAND(1) HIT(2)')
        if s == "quit":
            break
        print(s)
        try:
            #i,ts = p(s,0,[])
        	pkt = Ether(dst='00:04:00:00:00:00', type=0x1234) / P4blackjack(move=int(p), card1=new_card_values[random_numbers[0]], card2=new_card_values[random_numbers[1]], card3=new_card_values[random_numbers[2]], card4=new_card_values[random_numbers[3]], card5=new_card_values[random_numbers[4]], card6=new_card_values[random_numbers[5]], card7=new_card_values[random_numbers[6]], card8=new_card_values[random_numbers[7]], card9=new_card_values[random_numbers[8]], card10=new_card_values[random_numbers[9]], card11=new_card_values[random_numbers[10]], card12=new_card_values[random_numbers[11]], card13=new_card_values[random_numbers[12]], card14=new_card_values[random_numbers[13]], card15=new_card_values[random_numbers[14]])
            ...
            if resp:
                ...
                if blackjack.res == 0:
                	if input_value in card_values:
        				print(card_values[blackjack.card1] card_values[blackjack.card2])
        			
    				#else:
        				#print(f"No card found for {hex(input_value)}")
